-- Script complet pour corriger tous les problèmes de recommandations
...
-- 5. Créer la table history avec contrainte unique corrigée
CREATE TABLE public.history (
    ...
    -- Contrainte unique corrigée (sans fonction DATE)
    UNIQUE(user_id, product_id, viewed_at::date)
);
...
-- 9. Fonction pour ajouter une consultation initiale (corrigée)
...
AS $$
BEGIN
    -- Insérer ou mettre à jour l'entrée avec la contrainte unique
    ...
    ON CONFLICT (user_id, product_id, viewed_at::date)
    DO UPDATE SET 
        viewed_at = NOW(),
        last_updated = NOW(),
...
SELECT 'Tous les problèmes de recommandations corrigés avec succès' as status; 