-- MIGRATION DES DONNÉES DE SAUVEGARDE VERS LES NOUVELLES TABLES
...
-- =====================================================
-- ÉTAPE 2: MIGRATION DES COMMANDES
-- =====================================================
...
-- Insérer les commandes de sauvegarde avec de nouveaux UUIDs
...
RETURNING id, (SELECT id FROM public.orders_backup_manual WHERE id = id) as old_id;
...
JOIN public.orders_backup_manual ob ON 
    o.prix_total = ob.prix_total AND 
    o.user_id = ob.user_id AND 
    o.created_at = ob.created_at;
...
-- Optionnel : Supprimer les tables de sauvegarde si tout s'est bien passé
-- DÉCOMMENTEZ LES LIGNES SUIVANTES SI VOUS VOULEZ SUPPRIMER LES SAUVEGARDES
-- DROP TABLE IF EXISTS public.orders_backup_manual;
-- DROP TABLE IF EXISTS public.order_variants_backup_manual;
-- DROP TABLE IF EXISTS public.payments_backup_manual;
