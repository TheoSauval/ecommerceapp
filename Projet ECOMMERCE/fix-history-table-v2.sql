-- Script de correction pour la table history - Version 2
-- Solution alternative sans fonction dans la contrainte UNIQUE
...
-- 5. Recréer la table history sans contrainte UNIQUE problématique
...
-- 6. Créer un index unique sur la date (sans fonction)
CREATE UNIQUE INDEX idx_history_user_product_date 
    ON public.history (user_id, product_id, viewed_at::date);

-- 7. Créer les autres index
...
-- 10. Créer la fonction pour ajouter une consultation (version simplifiée)
...
AS $$
BEGIN
    -- Vérifier si une consultation existe déjà pour aujourd'hui
    IF EXISTS (
        ...
        -- Mettre à jour l'heure de consultation
...
SELECT 'Table history créée avec succès - Version 2' as status; 