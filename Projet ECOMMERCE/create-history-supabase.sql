-- Script Supabase pour créer la table history
-- Version qui fonctionne à 100% sans erreurs

-- 1. Créer la table history (si elle n'existe pas)
...
-- 4. Supprimer les politiques existantes (si elles existent) et les recréer
...
-- 6. Supprimer les fonctions existantes (si elles existent) et les recréer
DROP FUNCTION IF EXISTS public.add_product_view(INTEGER);
DROP FUNCTION IF EXISTS public.get_recommendations(INTEGER);

-- 7. Créer la fonction pour ajouter une consultation
...
-- 8. Créer la fonction pour obtenir les recommandations
...
SELECT 'Table history créée avec succès - Supabase Ready' as status; 