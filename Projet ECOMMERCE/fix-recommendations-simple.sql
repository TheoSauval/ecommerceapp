-- Script simplifié pour corriger les problèmes de recommandations
-- Version sans contrainte unique complexe
...
-- 5. Créer la table history simplifiée (sans contrainte unique complexe)
...
-- 9. Fonction pour ajouter une consultation initiale (simplifiée)
CREATE OR REPLACE FUNCTION public.add_product_view(user_id_param UUID, product_id_param UUID)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    -- Insérer une nouvelle consultation (pas de conflit)
    INSERT INTO public.history (user_id, product_id, viewed_at, view_duration, created_at)
    VALUES (user_id_param, product_id_param, NOW(), 0, NOW());
END;
$$;

-- 10. Fonction pour mettre à jour la durée de consultation
CREATE OR REPLACE FUNCTION public.update_view_duration(user_id_param UUID, product_id_param UUID, duration_seconds INTEGER)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- 12. Fonction avancée pour obtenir les recommandations (structure corrigée)
